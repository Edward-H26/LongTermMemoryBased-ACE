"""
CL-bench V5: Run baseline and ACE inference in parallel.

Default behavior:
- Clear v5 result artifacts
- Optionally clear Neo4j database
- Run baseline_v5 and ace_v5 inference in parallel
...
from benchmark.preflight_v5 import (
    build_estimate,
...
def _init_run_meta(path: str, args: argparse.Namespace) -> None:
    ...
    if not payload:
        payload = {
            "run_id": f"v5-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}",
...
def run_main_pipeline(args: argparse.Namespace) -> None:
    baseline_path = os.path.join(args.output_dir, "baseline_v5.jsonl")
    ace_path = os.path.join(args.output_dir, "ace_v5.jsonl")
    ace_metrics_path = os.path.join(args.output_dir, "ace_v5_metrics.json")
    run_meta_path = os.path.join(args.output_dir, "run_v5_meta.json")
    ...
    if args.clear_results:
        paths_to_clear = [
            ...
            os.path.join(args.output_dir, "baseline_v5_graded.jsonl"),
    ...
    print("Starting baseline v5 and ACE v5 in parallel...")
...
def main() -> None:
    parser = argparse.ArgumentParser(description = "CL-bench V5: Parallel baseline + ACE inference")
    parser.add_argument("--manifest", type = str, default = "benchmark/results/v5/subset_manifest_v5_seed42_n200.json")
    ...
    parser.add_argument(
        ...
        default = True,
        help = "Delete v5 result files before starting (default: True). Use --no-clear-results to resume.",
    )
    parser.add_argument(
        "--clear-db",
        action = argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        "--smoke-output-dir",
        type = str,
        default = "benchmark/results/v5/smoke_v5",
    ...
    parser.add_argument(
        ...
        type = str,
        default = "auto",
        choices = ["auto", "v5", "v4", "v3", "heuristic"],
        help = "Source to use for static cost/time estimation.",
    )
    parser.add_argument(
        "--preflight-report",
        type = str,
        default = "benchmark/results/v5/preflight_v5.json",
    ...
    parser.add_argument(
        ...
        action = argparse.BooleanOptionalAction,
        default = False,
        help = "Run benchmark.sanitize_results after pipeline completion.",
    )
    parser.add_argument(
        "--sanitize-in-place",
        action = argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        "--sanitize-report",
        type = str,
        default = "benchmark/results/v5/sanitize_report_v5.json",
    ...
    static_checks = run_static_checks(
        ...
        progress_path = os.path.join(args.output_dir, "ace_v5.jsonl.progress.jsonl"),
