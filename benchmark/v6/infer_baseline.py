"""
CL-bench Baseline Inference V6: Raw GPT-5.1, no ACE.

V6 keeps baseline behavior stable while renaming artifacts for isolated runs.

Usage:
    python -m benchmark.v6.infer_baseline \
        --max-samples 200 \
        --seed 42 \
        --manifest benchmark/results/v6/subset_manifest_v6_seed42_n200.json \
        --sampling-strategy context_dense \
        --max-completion-tokens 8192 \
        --empty-output-retry-max-tokens 16384 \
        --output benchmark/results/v6/baseline_v6.jsonl
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description = "CL-bench Baseline V6 Inference")
    parser.add_argument("--model", type = str, default = "gpt-5.1")
    parser.add_argument("--output", type = str, default = "benchmark/results/v6/baseline_v6.jsonl")
    ...
    for item in tqdm(pending, desc = "Baseline V6"):
