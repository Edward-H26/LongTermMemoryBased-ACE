"""
Replay terminal benchmark rewards into V6 planner policy state.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description = "Replay V6 graded rewards into planner policy state")
    parser.add_argument("--baseline-graded", type = str, required = True)
    parser.add_argument("--ace-graded", type = str, required = True)
    parser.add_argument("--output", type = str, default = "benchmark/results/v6/policy_replay_v6.json")
    ...
    baseline_policy_path = args.baseline_policy_state or os.path.join(args.output_dir, "planner_policy_baseline_v6.json")
    ace_policy_path = args.ace_policy_state or os.path.join(args.output_dir, "planner_policy_ace_v6.json")
    ...
    if args.mode in {"both", "baseline"}:
        ...
        baseline_summary = replay_rows(baseline_rows, baseline_planner, stream = "baseline_v6")
    ...
    if args.mode in {"both", "ace"}:
        ...
        ace_summary = replay_rows(ace_rows, ace_planner, stream = "ace_v6")
    ...
    if baseline_summary:
        baseline_rows = baseline_summary.get("rows", 0)
        baseline_updates = baseline_summary.get("updates", 0)
        baseline_mean_reward = baseline_summary.get("mean_terminal_reward", 0.0)
        print(
            "Baseline replay:",
            f"rows={baseline_rows}",
            f"updates={baseline_updates}",
            f"mean_terminal_reward={baseline_mean_reward:.4f}",
        )
    if ace_summary:
        ace_rows = ace_summary.get("rows", 0)
        ace_updates = ace_summary.get("updates", 0)
        ace_mean_reward = ace_summary.get("mean_terminal_reward", 0.0)
        print(
            "ACE replay:",
            f"rows={ace_rows}",
            f"updates={ace_updates}",
            f"mean_terminal_reward={ace_mean_reward:.4f}",
        )
