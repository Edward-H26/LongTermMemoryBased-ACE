"""
Wait for v5 inference to complete, then run eval, error analysis, and comparison report.
Run after infer_baseline_v5 and infer_ace_direct_v5 (or run_v5).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description = "V5 post-inference: eval, error analysis, compare")
    ...
    parser.add_argument(
        "--skip-wait",
        action = "store_true",
        help = "Skip polling; assume inference is already complete (used when invoked by run_v5).",
    )
    ...
    baseline_graded_path = os.path.join(args.output_dir, "baseline_v5_graded.jsonl")
    ace_graded_path = os.path.join(args.output_dir, "ace_v5_graded.jsonl")
    baseline_errors_path = os.path.join(args.output_dir, "baseline_v5_graded_errors.jsonl")
    ace_errors_path = os.path.join(args.output_dir, "ace_v5_graded_errors.jsonl")
    baseline_eval_metrics_path = os.path.join(args.output_dir, "baseline_v5_graded_eval_metrics.json")
    ...
    if not args.skip_wait:
        print(f"Waiting for baseline_v5.jsonl and ace_v5.jsonl to reach {args.max_samples} lines...")
    while True:
    ...
    subprocess.run(
        [
            ...
            ace_errors_path,
            "--ace-aux-metrics",
            os.path.join(args.output_dir, "ace_v5_metrics.json"),
