"""
CL-bench ACE Direct Inference V5: GPT-5.1 + ACE memory enrichment.

V5 additions:
- Memory scope parameter: hybrid, local, global
- Dual memory channels with local + global retrieval
- Context-level parallel execution with deterministic output finalization
- Capped-output retry for empty completion-capped responses
- Per-task step-level process scoring diagnostics
- Per-task durable progress journal for crash-safe resume
- Fail-soft memory operations with write retry diagnostics
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description = "CL-bench ACE Direct V5 Inference")
    parser.add_argument("--output", type = str, default = "benchmark/results/v5/ace_v5.jsonl")
    ...
    parser.add_argument(
        ...
        action = argparse.BooleanOptionalAction,
        default = _safe_env_bool("ACE_V5_RESUME_FROM_PROGRESS", True),
        help = "Resume from progress journal in addition to output file (default: True).",
    )
    parser.add_argument(
        "--finalize-order",
        action = argparse.BooleanOptionalAction,
        default = _safe_env_bool("ACE_V5_FINALIZE_ORDER", True),
    ...
    def process_context_tasks(context_id: str, tasks: List[Dict[str, Any]]) -> Tuple[List[Tuple[int, Dict[str, Any]]], int]:
        ...
        for item in tasks:
            ...
            if not messages:
                context_failures += 1
                failure_row = create_failure_result(item, task_id, "missing_messages", args.memory_scope)
            ...
            if ace_delta_local.get("persist_error"):
                memory_errors.append(f"local_persist:{ace_delta_local.get('persist_error')}")
            if ace_delta_global.get("persist_error"):
                memory_errors.append(f"global_persist:{ace_delta_global.get('persist_error')}")
    ...
    progress = tqdm(total = len(pending), desc = "ACE V5")
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                for item in context_tasks:
                    task_id = get_task_id(item)
                    index = task_index.get(task_id, 10**9)
                    failed_row = create_failure_result(item, task_id, f"context_worker_exception:{exc}", args.memory_scope)
    ...
    print(f"\nDone: {final_stats['rows']} finalized rows, {total_failures} with API/format failures")
